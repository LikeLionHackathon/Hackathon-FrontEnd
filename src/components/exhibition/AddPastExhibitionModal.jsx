...
function SearchArtist({ pendingPick, setPendingPick }) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setErr(null);
      try {
        const { data } = await axiosInstance.get(`/api/v1/users/${debounced}`, {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center mt-[20px] justify-between px-2 py-2 rounded-lg hover:bg-grey01/40  ${
                picked
                  ? 'text-[14px] font-[600] leading-1.5 ring-2 ring-purple_main'
                  : ''
              }`}
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-[16px] leading-1.5 font-[600]">
                  {u.name}
                </span>
              </div>
              <div>
                {picked ? (
                  <img src={icon_addfriend2} />
                ) : (
                  <img src={icon_addfriend} />
                )}
              </div>
            </button>
          );
...
export default function AddPastExhibitionModal({ open, onClose, onSubmit }) {
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      try {
        const newExhibitionId = await createMyExhibition(data);

        // ✅ 1. 이 로그가 찍히는지, newExhibitionId가 숫자로 나오는지 확인하세요.
        console.log('🚀 다음 페이지로 전달할 ID:', newExhibitionId);

...
import { postUserPreferences } from '../apis/userPreference';
...
export default function Onboarding() {
  ...
  const STEPS = useMemo(() => [
    ...
    { questionId: 1, key: 'interests', title: '관심사를 선택하세요', note: '(복수 선택 가능)', multi: true, min: 1, options: [
        { num: 1, id: 'nature', title: '자연' }, { num: 2, id: 'city', title: '도시' }, { num: 3, id: 'figure', title: '인물' }, { num: 4, id: 'memory', title: '추억' },
        { num: 5, id: 'history', title: '역사' }, { num: 6, id: 'religion', title: '종교' }, { num: 7, id: 'social', title: '사회비평' }, { num: 8, id: 'korea', title: '한국' },
        { num: 9, id: 'modern', title: '현대' }, { num: 10, id: 'environment', title: '환경' },
    ]},
    { questionId: 2, key: 'genres', title: '어떤 장르의 전시를 좋아하시나요?', note: '(복수 선택 가능)', multi: true, min: 1, options: [
        { num: 1, id: 'contemporary', title: '현대미술' }, { num: 2, id: 'installation', title: '설치미술' }, { num: 3, id: 'digital', title: '디지털아트' },
        { num: 4, id: 'graphic', title: '그래픽' }, { num: 5, id: 'video', title: '영상' }, { num: 6, id: 'photo', title: '사진' },
        { num: 7, id: 'interactive', title: '인터랙티브 아트' }, { num: 8, id: 'ceramics', title: '도자기' }, { num: 9, id: 'glass', title: '유리공예' }, { num: 10, id: 'oriental', title: '동양화' },
    ]},
    { questionId: 3, key: 'moods', title: '전시장에서 어떤 분위기를 좋아하시나요?', note: '(복수 선택 가능)', multi: true, min: 1, options: [
        { num: 1, id: 'calm', title: '차분하고 사색적인', image: mood1 }, { num: 2, id: 'dense', title: '몰입감 있고 밀도적인', image: mood2 },
        { num: 3, id: 'warm', title: '따뜻하고 낭만적인', image: mood3 }, { num: 4, id: 'experimental', title: '실험적이고 도전적인', image: mood4 },
    ]},
    { questionId: 4, key: 'companions', title: '전시는 보통 누구와 함께 즐기시나요?', note: '(복수 선택 가능)', multi: true, min: 1, options: [
        { num: 1, id: 'solo', title: '혼자 집중해서', image: who1, sub: 1 }, { num: 2, id: 'lover', title: '연인과 함께', image: who2, sub: 1  },
        { num: 3, id: 'friends', title: '친구와 함께', image: who3, sub: 1  }, { num: 4, id: 'family', title: '가족과 함께', image: who4, sub: 1  },
    ]},
  ], []);
  ...
  async function completeSignup(payload) {
    const preferenceAnswers = STEPS.slice(1).map(stepInfo => {
        const answers = payload[stepInfo.key];
        return {
            questionId: stepInfo.questionId,
            answerId: answers
        };
    });
        ...
        return (
          <div className="w-full h-full pt-4">
            <div className="grid grid-cols-2 gap-3">
              {cur.options.map((op) => (
                <SimpleTile type={2} key={op.id} title={op.title} selected={form[key].includes(op.num)} onClick={() => toggle(key, op.num, true)} />
              ))}
            </div>
          </div>
        );
      ...
      case 'companions':
        return (
          <div className="w-full h-full pt-4">
            <div className="grid grid-cols-2 gap-3">
              {cur.options.map((op) => (
                <Tile type={3} key={op.id} title={op.title} selected={form[key].includes(op.num)} onClick={() => toggle(key, op.num, true)} image={op.image} sub={op.sub}/>
              ))}
            </div>
          </div>
        );
